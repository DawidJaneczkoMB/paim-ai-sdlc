...
description: 'Use when creating reusable Terraform modules, standardizing AWS cloud provisioning, implementing IaC best practices, or establishing organizational Terraform module standards.'
---

# Terraform Module Library (AWS)

Production-ready Terraform module patterns for AWS infrastructure.

## When to Use

- Build reusable infrastructure components for AWS
...
2. **Pin provider versions** in versions.tf — use `aws ~> 6.0`
...
- Always use `runtime.Caller(0)` to resolve `examples/complete/` path relative to the test file
- Always use `single_nat_gateway = true` (or equivalent cost-reducing flags) in test examples
- Provide a **plan-only** variant (using `InitAndPlanAndShowWithStruct`) for fast PR validation without AWS credentials
...
- `references/aws-modules.md` — AWS module patterns (VPC, EKS, RDS, S3, ALB, Lambda, Security Groups)

## Connected Skills

- `optimize-cloud-cost` — for cost-effective infrastructure designs
- `architecture-design` — for architectural decisions before module implementation
- `create-implementation-plan` — for planning module rollout across environments
