...
Look up platform-specific syntax via web search with exact version numbers.
...
Each stage should be independent and cacheable.
...
**Rule:** Prefer OIDC federation over long-lived access keys.
...
1. **Discover context** → Use `common-technical-context-discovery` to find existing CI patterns
2. **Detect platform** → Check for CI config files listed above
3. **Look up syntax** → Web search for platform-specific YAML syntax with version
4. **Choose deployment strategy** → Use decision table above
5. **Configure credentials** → OIDC preferred; platform secrets as fallback
